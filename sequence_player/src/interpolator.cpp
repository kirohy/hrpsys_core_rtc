...
void interpolator::linear_interpolation(double &remain_t_, double gx, double &xx, double &vv, double &aa) {
    if (remain_t_ > dt + EPS) {
        aa = 0;
        vv = (gx - xx) / remain_t_;
        xx += vv * dt;
...
bool interpolator::setInterpolationMode(interpolation_mode i_mode_) {
    if (i_mode_ != LINEAR && i_mode_ != HOFFARBIB && i_mode_ != QUINTICSPLINE && i_mode_ != CUBICSPLINE) return false;
    imode = i_mode_;
    return true;
};

void interpolator::setGoal(const double *newg, double time, bool online) { setGoal(newg, NULL, time, online); }

void interpolator::setGoal(const double *newg, const double *newv, double time, bool online) {
    std::memcpy(gx, newg, sizeof(double) * dim);
    if (newv != NULL)
        std::memcpy(gv, newv, sizeof(double) * dim);
    else {
        for (int i = 0; i < dim; i++) {
            gv[i] = 0;
        }
...
void interpolator::interpolate(double &remain_t_) {
    ...
    for (int i = 0; i < dim; i++) {
        ...
        switch (imode) {
        case LINEAR:
            linear_interpolation(tm, gx[i], x[i], v[i], a[i]);
            break;
        case HOFFARBIB:
        case QUINTICSPLINE:
        case CUBICSPLINE:
            hoffarbib(tm, a0[i], a1[i], a2[i], a3[i], a4[i], a5[i], x[i], v[i], a[i]);
            break;
        }
    }
    push(x, v, a);
...
void interpolator::go(const double *newg, double time, bool immediate) { go(newg, NULL, time, immediate); }
...
void interpolator::push(const double *x_, const double *v_, const double *a_, bool immediate) {
    ...
    std::memcpy(p, x_, sizeof(double) * dim);
    std::memcpy(dp, v_, sizeof(double) * dim);
    std::memcpy(ddp, a_, sizeof(double) * dim);
    q.push_back(p);
    dq.push_back(dp);
    ddq.push_back(ddp);
    if (immediate) sync();
...
void interpolator::pop_back() {
    ...
    if (length > 0) {
        ...
        q.pop_back();
        if (length > 0) {
            std::memcpy(x, q.back(), sizeof(double) * dim);
        } else {
            std::memcpy(x, gx, sizeof(double) * dim);
        }
        double *&dvs = dq.back();
        delete[] dvs;
        dq.pop_back();
        if (length > 0) {
            std::memcpy(v, dq.back(), sizeof(double) * dim);
        } else {
            std::memcpy(v, gv, sizeof(double) * dim);
        }
        double *&ddvs = ddq.back();
        delete[] ddvs;
        ddq.pop_back();
        if (length > 0) {
            std::memcpy(a, ddq.back(), sizeof(double) * dim);
        } else {
            std::memcpy(a, ga, sizeof(double) * dim);
        }
    } else if (remain_t > 0) {
        remain_t = 0;
    }
...
void interpolator::get(double *x_, bool popp) { get(x_, NULL, NULL, popp); }

void interpolator::get(double *x_, double *v_, bool popp) { get(x_, v_, NULL, popp); }
...
void interpolator::get(double *x_, double *v_, double *a_, bool popp) {
    ...
    if (length != 0) {
        ...
        if (a_ != NULL) std::memcpy(a_, ddvs, sizeof(double) * dim);
        if (popp) pop();
    } else {
        std::memcpy(x_, gx, sizeof(double) * dim);
        if (v_ != NULL) std::memcpy(v_, gv, sizeof(double) * dim);
        if (a_ != NULL) std::memcpy(a_, ga, sizeof(double) * dim);
    }
}

bool interpolator::isEmpty() { return length == 0 && remain_t <= 0; }

double interpolator::remain_time() { return dt * length; }
