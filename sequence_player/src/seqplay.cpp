...
seqplay::seqplay(unsigned int i_dof, double i_dt, unsigned int i_fnum, unsigned int optional_data_dim) : m_dof(i_dof) {
    ...
    interpolators[Q]             = std::make_shared<interpolator>(i_dof, i_dt);
    interpolators[ZMP]           = std::make_shared<interpolator>(3, i_dt);
    interpolators[ACC]           = std::make_shared<interpolator>(3, i_dt);
    ...
    for (size_t i = 0; i < 6 * i_fnum; i++)
        initial_wrenches[i] = 0;
    interpolators[WRENCHES]->set(initial_wrenches);
    double initial_optional_data[optional_data_dim];
    for (size_t i = 0; i < optional_data_dim; i++)
        initial_optional_data[i] = 0;
...
bool seqplay::isEmpty() const {
    ...
    for (auto it : groupInterpolators) {
        auto gi = it.second;
...
void seqplay::getJointAngles(double *jvs) { interpolators[Q]->get(jvs, false); }
...
void seqplay::playPattern(std::vector<const double *> pos, std::vector<const double *> zmp, std::vector<const double *> rpy, std::vector<double> tm,
    ...
    for (unsigned int i = 0; i < pos.size(); i++) {
        ...
        if (i < tm.size()) t = tm[i];
        go(q, z, a, p, e, tq, wr, od, v, NULL, NULL, NULL, NULL, NULL, NULL, NULL, t, false);
    }
    sync();
    delete[] v;
}
...
void seqplay::clear(double i_timeLimit) {
    tick_t t1 = get_tick();
    while (!isEmpty()) {
        if (i_timeLimit > 0 && tick2sec(get_tick() - t1) >= i_timeLimit) break;
        pop_back();
    }
}
...
void seqplay::loadPattern(const char *basename, double tm) {
    double scale = 1.0;
    bool found   = false;
    if (debug_level > 0) std::cout << "pos   = ";
    std::string pos = basename;
    pos.append(".pos");
    ...
    if (debug_level > 0) std::cout << std::endl << "zmp   = ";
    std::string zmp = basename;
    zmp.append(".zmp");
    if (access(zmp.c_str(), 0) == 0) {
        found = true;
        interpolators[ZMP]->load(zmp, tm, scale, false);
        if (debug_level > 0) std::cout << zmp;
    }
    if (debug_level > 0) std::cout << std::endl << "gsens = ";
    std::string acc = basename;
    acc.append(".gsens");
    if (access(acc.c_str(), 0) == 0) {
        found = true;
        interpolators[ACC]->load(acc, tm, scale, false);
        if (debug_level > 0) std::cout << acc;
    }
    if (debug_level > 0) std::cout << std::endl << "hip   = ";
    std::string hip = basename;
    hip.append(".hip");
    if (access(hip.c_str(), 0) == 0) {
        found = true;
        interpolators[RPY]->load(hip, tm, scale, false);
        if (debug_level > 0) std::cout << hip;
    } else {
        hip = basename;
        hip.append(".waist");
        if (access(hip.c_str(), 0) == 0) {
    ...
    if (debug_level > 0) std::cout << std::endl << "torque = ";
    std::string torque = basename;
    torque.append(".torque");
    if (access(torque.c_str(), 0) == 0) {
        found = true;
        interpolators[TQ]->load(torque, tm, scale, false);
        if (debug_level > 0) std::cout << torque;
    }
    if (debug_level > 0) std::cout << std::endl << "wrenches   = ";
    std::string wrenches = basename;
    wrenches.append(".wrenches");
    if (access(wrenches.c_str(), 0) == 0) {
        found = true;
        interpolators[WRENCHES]->load(wrenches, tm, scale, false);
        if (debug_level > 0) std::cout << wrenches;
    }
    if (debug_level > 0) std::cout << std::endl << "optional_data   = ";
    std::string optional_data = basename;
    optional_data.append(".optionaldata");
    if (access(optional_data.c_str(), 0) == 0) {
        found = true;
        interpolators[OPTIONAL_DATA]->load(optional_data, tm, scale, false);
        if (debug_level > 0) std::cout << optional_data;
    }
    if (debug_level > 0) std::cout << std::endl;
    if (!found) std::cerr << "pattern not found(" << basename << ")" << std::endl;
    //
    sync();
}
...
bool seqplay::addJointGroup(const char *gname, const std::vector<int> &indices) {
    ...
    i                         = std::make_shared<groupInterpolator>(indices, interpolators[Q]->deltaT());
...
bool seqplay::setJointAnglesSequenceFull(std::vector<const double *> i_pos, std::vector<const double *> i_vel, std::vector<const double *> i_torques,
    ...
    for (int j = 0; j < optional_data_dim; j++) {
        dummy_optional[j] = 0.0;
    }
    ...
    for (unsigned int i = 0; i < i_pos.size(); i++) {
        ...
        do {
            ...
            tm_tmp = tm;
            interpolators[TQ]->interpolate(tm_tmp);
            tm_tmp = tm;
            interpolators[P]->interpolate(tm_tmp);
            tm_tmp = tm;
            interpolators[RPY]->interpolate(tm_tmp);
            tm_tmp = tm;
            interpolators[ACC]->interpolate(tm_tmp);
            tm_tmp = tm;
            interpolators[ZMP]->interpolate(tm_tmp);
            tm_tmp = tm;
            interpolators[WRENCHES]->interpolate(tm_tmp);
            tm_tmp = tm;
            interpolators[OPTIONAL_DATA]->interpolate(tm_tmp);
        } while (i_tm[i] > 0);
        sync();
    }
    return true;
