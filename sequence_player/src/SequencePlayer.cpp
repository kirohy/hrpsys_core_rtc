...
SequencePlayer::SequencePlayer(RTC::Manager *manager)
    : RTC::DataFlowComponentBase(manager), m_qInitIn("qInit", m_qInit), m_basePosInitIn("basePosInit", m_basePosInit),
      ...
      m_optionalDataOut("optionalData", m_optionalData), m_SequencePlayerServicePort("SequencePlayerService"), m_robot(cnoid::BodyPtr()), m_debugLevel(0),
...
RTC::ReturnCode_t SequencePlayer::onInitialize() {
    ...
    for (unsigned int i = 0; i < nforce; i++) {
        m_wrenchesOut[i] = new RTC::OutPort<RTC::TimedDoubleSeq>(std::string(fsensor_names[i] + "Ref").c_str(), m_wrenches[i]);
...
bool SequencePlayer::setJointAngles(const double *angles, const bool *mask, double tm) {
    if (m_debugLevel > 0) { RTC_INFO_STREAM(__PRETTY_FUNCTION__); }
    std::lock_guard<std::mutex> guard(m_mutex);
...
bool SequencePlayer::setJointAnglesSequenceFull(const OpenHRP::dSequenceSequence i_jvss, const OpenHRP::dSequenceSequence i_vels,
...
void SequencePlayer::setMaxIKIteration(short iter) { m_iteration = iter; }

bool SequencePlayer::getProperty(const std::string &key, std::string &ret) {
...
void SequencePlayerInit(RTC::Manager *manager) {
    ...
    manager->registerFactory(profile, RTC::Create<SequencePlayer>, RTC::Delete<SequencePlayer>);
}
};
