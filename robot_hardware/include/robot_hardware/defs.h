...
template <class T> void LIMITER(T &org, const T &min_, const T &max_) {
...
template <class T> void DEADZONE(T &org, const T &min_, const T &max_) {
    for (unsigned int i = 0; i < org.size(); i++) {
        if (org(i) > max_(i)) {
            org(i) -= max_(i);
        } else if (org(i) < min_(i)) {
...
inline void HYSTERESIS(double &newv, const double old, const double min_, const double max_) {
    if (old >= newv - min_) {
        newv -= min_;
