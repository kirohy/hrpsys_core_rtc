...
robot::robot(double dt)
    : m_fzLimitRatio(0), m_maxZmpError(DEFAULT_MAX_ZMP_ERROR), m_accLimit(0), m_calibRequested(false), m_pdgainsFilename("PDgains.sav"),
      m_reportedEmergency(true), m_dt(dt), m_enable_poweroff_check(false), m_servoOnDelay(0) {
    sem_init(&wait_sem, 0, 0);
    m_rLegForceSensorId = m_lLegForceSensorId = -1;
}

robot::~robot() { close_iob(); }
...
bool robot::init() {
    ...
    force_sensors_ << this->devices();
    gyro_sensors_ << this->devices();
    acc_sensors_ << this->devices();
    set_number_of_force_sensors(this->numForceSensors());
    set_number_of_gyro_sensors(this->numGyroSensors());
    set_number_of_accelerometers(this->numAccSensors());

    force_sum.resize(this->numForceSensors());
    gyro_sum.resize(this->numGyroSensors());
    accel_sum.resize(this->numAccSensors());

    if ((number_of_joints() != (int)numJoints()) || (number_of_force_sensors() != (int)this->numForceSensors()) ||
        (number_of_gyro_sensors() != (int)this->numGyroSensors()) || (number_of_accelerometers() != (int)this->numAccSensors())) {
        std::cerr << "[RobotHardware] VRML and IOB are inconsistent" << std::endl;
        std::cerr << "[RobotHardware]   joints:" << numJoints() << "(VRML), " << number_of_joints() << "(IOB)" << std::endl;
        std::cerr << "[RobotHardware]   force sensor:" << this->numForceSensors() << "(VRML), " << number_of_force_sensors() << "(IOB)" << std::endl;
        std::cerr << "[RobotHardware]   gyro sensor:" << this->numGyroSensors() << "(VRML), " << number_of_gyro_sensors() << "(IOB)" << std::endl;
        std::cerr << "[RobotHardware]   accelerometer:" << this->numAccSensors() << "(VRML), " << number_of_accelerometers() << "(IOB)" << std::endl;
        return false;
    }
    G << 0, 0, 9.8;
...
bool robot::loadGain() {
    ...
    if (!strm.is_open()) {
        std::cerr << "[RobotHardware] " << m_pdgainsFilename << " not found" << std::endl;
        return false;
    }
    ...
    for (int i = 0; i < numJoints(); i++) {
        ...
        if (!getlinep) {
            if (i < numJoints()) {
                std::cerr << "[RobotHardware] loadGain error: size of gains reading from file (" << m_pdgainsFilename << ") does not match size of joints"
                          << std::endl;
            }
            break;
        }
    ...
    for (unsigned int i = 0; i < numJoints(); i++) {
        std::cerr << "[RobotHardware]   " << joint(i)->name() << ", pgain = " << default_pgain[i] << ", dgain = " << default_dgain[i]
                  << ", tqpgain = " << default_tqpgain[i] << ", tqdgain = " << default_tqdgain[i] << std::endl;
    }
    return true;
}
...
void robot::startInertiaSensorCalibration() {
    ...
    if (this->numAccSensors() == 0 && this->numGyroSensors() == 0) return;

    if (isBusy()) return;

    for (unsigned int j = 0; j < this->numGyroSensors(); j++) {
        for (int i = 0; i < 3; i++) {
            gyro_sum[j][i] = 0;
        }
    ...
    for (unsigned int j = 0; j < this->numAccSensors(); j++) {
        for (int i = 0; i < 3; i++) {
            accel_sum[j][i] = 0;
        }
...
void robot::startForceSensorCalibration() {
    if (this->numForceSensors() == 0) return;

    if (isBusy()) return;

    for (unsigned int j = 0; j < this->numForceSensors(); j++) {
        for (int i = 0; i < 6; i++) {
            force_sum[j][i] = 0;
        }
    }
...
void robot::calibrateInertiaSensorOneStep() {
    if (inertia_calib_counter > 0) {
        ...
        if (inertia_calib_counter == 0) {
            ...
            for (unsigned int j = 0; j < this->numAccSensors(); j++) {
                cnoid::AccelerationSensorPtr m_sensor = acc_sensors_[j];
                cnoid::Matrix3 m_sensorR              = m_sensor->link()->R() * m_sensor->R_local();
                cnoid::Vector3 G_rotated              = m_sensorR.transpose() * G;
                for (int i = 0; i < 3; i++) {
...
void robot::oneStep() {
    ...
    gain_control();
    if (m_calibRequested) {
        ::initializeJointAngle(m_calibJointName.c_str(), m_calibOptions.c_str());
...
bool robot::isBusy() const {
    if (inertia_calib_counter > 0 || force_calib_counter > 0) return true;

    for (unsigned int i = 0; i < numJoints(); i++) {
        if (gain_counter[i] < max_gain_count[i]) { return true; }
    }

    return false;
}

void robot::readJointAngles(double *o_angles) { read_actual_angles(o_angles); }

void robot::readJointVelocities(double *o_velocities) { read_actual_velocities(o_velocities); }

int robot::readJointTorques(double *o_torques) { return read_actual_torques(o_torques); }

int robot::readJointCommandTorques(double *o_torques) { return read_command_torques(o_torques); }

void robot::readGyroSensor(unsigned int i_rank, double *o_rates) { read_gyro_sensor(i_rank, o_rates); }

void robot::readAccelerometer(unsigned int i_rank, double *o_accs) { read_accelerometer(i_rank, o_accs); }

void robot::readForceSensor(unsigned int i_rank, double *o_forces) { read_force_sensor(i_rank, o_forces); }
...
void robot::readJointCommands(double *o_commands) { read_command_angles(o_commands); }

void robot::writeTorqueCommands(const double *i_commands) { write_command_torques(i_commands); }

void robot::writeVelocityCommands(const double *i_commands) { write_command_velocities(i_commands); }
...
void robot::readPowerStatus(double &o_voltage, double &o_current) { read_power(&o_voltage, &o_current); }
        ...
        if (state == ON) {
            ...
            if (fabs(v) > joint(i)->dq_upper()) {
                std::cerr << time_string() << ": joint command velocity limit over: joint = " << joint(i)->name()
                          << ", vlimit = " << joint(i)->dq_upper() / M_PI * 180 << "[deg/s], v = " << v / M_PI * 180 << "[deg/s]" << std::endl;
                return true;
            }
            ...
            if (m_accLimit && fabs(a) > m_accLimit) {
                std::cerr << time_string() << ": joint command acceleration limit over: joint = " << joint(i)->name()
                          << ", alimit = " << m_accLimit / M_PI * 180 << "[deg/s^2], v = " << a / M_PI * 180 << "[deg/s^2]" << std::endl;
                return true;
            }
        }
    }
    ...
    for (unsigned int i = 0; i < numJoints(); i++) {
        ...
        if (state == ON && m_servoErrorLimit[i] != 0) {
            ...
            if (fabs(angle - command) > m_servoErrorLimit[i]) {
                std::cerr << time_string() << ": servo error limit over: joint = " << joint(i)->name() << ", qRef = " << command / M_PI * 180
                          << "[deg], q = " << angle / M_PI * 180 << "[deg]" << std::endl;
...
bool robot::setServoGainPercentage(const char *i_jname, double i_percentage, int mode, double transition_time) {
    if (i_percentage < 0 || 100 < i_percentage) {
        std::cerr << "[RobotHardware] Invalid percentage " << i_percentage << "[%] for setServoGainPercentage. Percentage should be in (0, 100)[%]."
                  << std::endl;
        return false;
    }
...
bool robot::setServoTorqueGainPercentage(const char *i_jname, double i_percentage) {
    if (i_percentage < 0 && 100 < i_percentage) {
        std::cerr << "[RobotHardware] Invalid percentage " << i_percentage << "[%] for setServoTorqueGainPercentage. Percentage should be in (0, 100)[%]."
                  << std::endl;
        return false;
    }
    cnoid::Link *l = nullptr;
...
size_t robot::lengthOfExtraServoState(int id) { return length_of_extra_servo_state(id); }

void robot::readExtraServoState(int id, int *state) { read_extra_servo_state(id, state); }
...
bool robot::writeDigitalOutput(const char *i_dout) { return write_digital_output(i_dout); }

bool robot::writeDigitalOutputWithMask(const char *i_dout, const char *i_mask) { return write_digital_output_with_mask(i_dout, i_mask); }

int robot::lengthDigitalOutput() { return length_digital_output(); }

bool robot::readDigitalOutput(char *o_dout) { return read_digital_output(o_dout); }

void robot::readBatteryState(unsigned int i_rank, double &voltage, double &current, double &soc) {
